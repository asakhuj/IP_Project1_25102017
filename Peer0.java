...
class Peer0RFC  {
	...
	public Peer0RFC(String a, String b, String c) {
			rfcnumber = a;
			rfcname = b;
			hostname = c;
		}
	
	static LinkedList<Peer0RFC> rfcList = new LinkedList<Peer0RFC>();
	@Override
	 ...
	 */
	public  boolean searchForFilesToDownload(LinkedList<Peer0RFC> myRFCIndex, String myPortNumber) {
		boolean areThereAnyFiles =false;
		
		if(myRFCIndex!=null) {
			for(Peer0RFC element : myRFCIndex) {
...
/**
 * @author Arpita
 *This class will run the server code for Peer0, with port number 65401
 */
class ServerPeer0 implements Runnable {
	
	public void run() {
		try {
			ServerSocket welcomeSocket = new ServerSocket(65401);
	     	while(true) {
				...
				if(data.equalsIgnoreCase("GET RFC-INDEX")) {
					...
					outToClient.writeBytes("Size : "+Peer0.arrFiles.size() +'\n');
				
					for(String fileName : Peer0.arrFiles) {
					...
					connectionSocket.close();
						...
						String myFolder = currentDirectory+File.separator+"Peer0"; 
...
public class Peer0 {
	...
	public static void main (String args[]) throws Exception {
			...
			String myPortNumber = "65401"; //Port number for Peer0
		...
		//Creating Local RFC-Index
				String currentDirectory = System.getProperty("user.dir");
				String myFolderPath = currentDirectory+File.separator+"Peer0";
				...
				for (int i = 0; i < listOfFiles.length; i++) {
				      	if (listOfFiles[i].isFile()) {
				      		...
				      		Peer0RFC peerObj = new Peer0RFC(rfc_number[0], listOfFiles[i].getName() ,myPortNumber); //Have to change
				      		Peer0RFC.rfcList.add(peerObj);
				      		arrFiles.add(fileName);
				      } 
				 
				    }
				    
				    System.out.println("Local RFC Index" +Peer0RFC.rfcList.toString());
				    
				    //Thread for the server process
				    ServerPeer0 obj = new ServerPeer0();
				  ...
				  //To check if the application is terminated
					...
					class ShutdownHook  extends Thread 
					{
					    public void run() 
					    {
						  ...
						  Peer0.appActive = false;  // boolean to check if the application is active
		...
		if(activePeersPresent) {
					...
					if(portNumbersOfActivePeers != null) {
							for(int element : portNumbersOfActivePeers) {
								...
								for(String fileName : titleList) {
									String rfc_number[] = fileName.split("\\.");
									Peer0RFC peerObj = new Peer0RFC(rfc_number[0], fileName ,element+""); //Have to change
									Peer0RFC.rfcList.add(peerObj);
			...
			System.out.println(Peer0RFC.rfcList);
	...
	//Download file code
  	...
  	sc = new Scanner(System.in);
	reply = sc.next();
	Peer0RFC obj2 = new Peer0RFC();
	if(reply.equalsIgnoreCase("y")) {
	//Main download code
		boolean otherFilesTobeDownloaded = obj2.searchForFilesToDownload(Peer0RFC.rfcList,myPortNumber);
		if(otherFilesTobeDownloaded) {
				String downloadFolder = myFolderPath; //Folder where the files will be downloaded
			  	...
			  	int bytesRead;
			  	int current;
			  	for(Peer0RFC abc : Peer0RFC.rfcList) 
			  	    {
			  	    ...
			  	    // receive file
				  	    ...
				  	    timeAppend += time3;
					  	String fileToStoreTimeStamps = currentDirectory+File.separator+"Peer0.text";
