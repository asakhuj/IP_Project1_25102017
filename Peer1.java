...
class Peer1RFC  {
	...
	public Peer1RFC(String a, String b, String c) {
			rfcnumber = a;
			rfcname = b;
			hostname = c;
		}
	
	static LinkedList<Peer1RFC> rfcList = new LinkedList<Peer1RFC>();
	@Override
	 ...
	 */
	public  boolean searchForFilesToDownload(LinkedList<Peer1RFC> myRFCIndex, String myPortNumber) {
		boolean areThereAnyFiles =false;
		
		if(myRFCIndex!=null) {
			for(Peer1RFC element : myRFCIndex) {
 ...
 */
class ServerPeer1 implements Runnable {
	
	public void run() {
		try {
			ServerSocket welcomeSocket = new ServerSocket(65402);
	     	while(true) {
				...
				if(data.equalsIgnoreCase("GET RFC-INDEX")) {
					...
					outToClient.writeBytes("Size : "+Peer1.arrFiles.size() +'\n');
				
					for(String fileName : Peer1.arrFiles) {
					...
					connectionSocket.close();
						...
						String myFolder = currentDirectory+File.separator+"Peer1"; 
...
public class Peer1 {
	...
	public static void main (String args[]) throws Exception {
			...
			String myPortNumber = "65402"; //Port number for Peer0
		...
		//Creating Local RFC-Index
				String currentDirectory = System.getProperty("user.dir");
				String myFolderPath = currentDirectory+File.separator+"Peer1";
				...
				for (int i = 0; i < listOfFiles.length; i++) {
				      	if (listOfFiles[i].isFile()) {
				      		...
				      		Peer1RFC peerObj = new Peer1RFC(rfc_number[0], listOfFiles[i].getName() ,myPortNumber); //Have to change
				      		Peer1RFC.rfcList.add(peerObj);
				      		arrFiles.add(fileName);
				      } 
				 
				    }
				    
				    System.out.println("Local RFC Index" +Peer1RFC.rfcList.toString());
				    
				    //Thread for the server process
				    ServerPeer1 obj = new ServerPeer1();
				  ...
				  //To check if the application is terminated
					...
					class ShutdownHook  extends Thread 
					{
					    public void run() 
					    {
						  ...
						  Peer1.appActive = false;  // boolean to check if the application is active
		...
		if(activePeersPresent) {
					...
					if(portNumbersOfActivePeers != null) {
							for(int element : portNumbersOfActivePeers) {
								...
								for(String fileName : titleList) {
									String rfc_number[] = fileName.split("\\.");
									Peer1RFC peerObj = new Peer1RFC(rfc_number[0], fileName ,element+""); //Have to change
									Peer1RFC.rfcList.add(peerObj);
			...
			System.out.println(Peer1RFC.rfcList);
	...
	//Download file code
  	...
  	sc = new Scanner(System.in);
	reply = sc.next();
	Peer1RFC obj2 = new Peer1RFC();
	if(reply.equalsIgnoreCase("y")) {
	//Main download code
		boolean otherFilesTobeDownloaded = obj2.searchForFilesToDownload(Peer1RFC.rfcList,myPortNumber);
		if(otherFilesTobeDownloaded) {
				String downloadFolder = myFolderPath; //Folder where the files will be downloaded
			  	...
			  	int bytesRead;
			  	int current;
			  	for(Peer1RFC abc : Peer1RFC.rfcList) 
			  	    {
			  	    ...
			  	    // receive file
				  	    ...
				  	    timeAppend += time3;
					  	String fileToStoreTimeStamps = currentDirectory+File.separator+"Peer1.text";
