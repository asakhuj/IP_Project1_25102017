...
class Peer2RFC  {
	...
	public Peer2RFC(String a, String b, String c) {
			rfcnumber = a;
			rfcname = b;
			hostname = c;
		}
	
	static LinkedList<Peer2RFC> rfcList = new LinkedList<Peer2RFC>();
	@Override
	...
	/**
	 * This method is used to check whether there are any files present to be downloaded in the local RFC.
	 * @param myRFCIndex of the local peer
	 * @param myPortNumber of the logged in peer
	 * @return true if files exists to be downloaded
	 */
	public  boolean searchForFilesToDownload(LinkedList<Peer2RFC> myRFCIndex, String myPortNumber) {
		boolean areThereAnyFiles =false;
		
		if(myRFCIndex!=null) {
			for(Peer2RFC element : myRFCIndex) {
...
/**
 * @author Arpita
 *This class will run the server code for Peer1, with port number 65402
 */
class ServerPeer2 implements Runnable {
	
	public void run() {
		try {
			ServerSocket welcomeSocket = new ServerSocket(65403);
	     	while(true) {
				...
				if(data.equalsIgnoreCase("GET RFC-INDEX")) {
					...
					outToClient.writeBytes("Size : "+Peer2.arrFiles.size() +'\n');
				
					for(String fileName : Peer2.arrFiles) {
					...
					connectionSocket.close();
						...
						String fileFinal = data.substring(12); //Fetch the file name
						String currentDirectory = System.getProperty("user.dir");
						String myFolder = currentDirectory+File.separator+"Peer2"; 
...
public class Peer2 {
	...
	public static void main (String args[]) throws Exception {
			...
			String myPortNumber = "65403"; //Port number for Peer0
		...
		//Creating Local RFC-Index
				String currentDirectory = System.getProperty("user.dir");
				String myFolderPath = currentDirectory+File.separator+"Peer2";
				File folder = new File(myFolderPath); //Folder for Local RFC
				...
				for (int i = 0; i < listOfFiles.length; i++) {
				      	if (listOfFiles[i].isFile()) {
				      		...
				      		Peer2RFC peerObj = new Peer2RFC(rfc_number[0], listOfFiles[i].getName() ,myPortNumber); //Have to change
				      		Peer2RFC.rfcList.add(peerObj);
				      		arrFiles.add(fileName);
				      } 
				 
				    }
				    
				    System.out.println("Local RFC Index" +Peer2RFC.rfcList.toString());
				    
				    //Thread for the server process
				    ServerPeer2 obj = new ServerPeer2();
				  ...
				  //To check if the application is terminated
					...
					class ShutdownHook  extends Thread 
					{
					    public void run() 
					    {
						  ...
						  Peer2.appActive = false;  // boolean to check if the application is active
		...
		if(activePeersPresent) {
					...
					if(portNumbersOfActivePeers != null) {
							for(int element : portNumbersOfActivePeers) {
								...
								for(String fileName : titleList) {
									String rfc_number[] = fileName.split("\\.");
									Peer2RFC peerObj = new Peer2RFC(rfc_number[0], fileName ,element+""); //Have to change
									Peer2RFC.rfcList.add(peerObj);
			...
			System.out.println(Peer2RFC.rfcList);
	...
	//Download file code
  	...
  	sc = new Scanner(System.in);
	reply = sc.next();
	Peer2RFC obj2 = new Peer2RFC();
	if(reply.equalsIgnoreCase("y")) {
	//Main download code
		boolean otherFilesTobeDownloaded = obj2.searchForFilesToDownload(Peer2RFC.rfcList,myPortNumber);
		if(otherFilesTobeDownloaded) {
				String downloadFolder = myFolderPath; //Folder where the files will be downloaded
			  	String SERVER = "127.0.0.1";  // localhost
			  	int bytesRead;
			  	int current;
			  	for(Peer2RFC abc : Peer2RFC.rfcList) 
			  	    {
			  	    ...
			  	    // receive file
				  	    ...
				  	    System.out.println("File " + downloadFolder+"/"+fileNameToBeDownloaded + " downloaded (" + content_length + " bytes read)");
				  	    long time2 = System.currentTimeMillis();
				  	    long time3 = time2-time1;
				  	    timeAppend += time3;
					  	String fileToStoreTimeStamps = currentDirectory+File.separator+"Peer2.text";
