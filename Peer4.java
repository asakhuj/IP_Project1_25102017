...
class Peer4RFC  {
	...
	public Peer4RFC(String a, String b, String c) {
	...
	static LinkedList<Peer4RFC> rfcList = new LinkedList<Peer4RFC>();
	@Override
	...
	public  boolean searchForFilesToDownload(LinkedList<Peer4RFC> myRFCIndex, String myPortNumber) {
		boolean areThereAnyFiles =false;
		
		if(myRFCIndex!=null) {
		for(Peer4RFC element : myRFCIndex) {
...
class ServerPeer4 implements Runnable {
	
	public void run() {
		try {
			ServerSocket welcomeSocket = new ServerSocket(65405);
			...
			while(true) {
				...
				if(data.equalsIgnoreCase("GET RFC-INDEX")) {
					...
					outToClient.writeBytes("Size : "+Peer4.arrFiles.size() +'\n');
				
					for(String abc : Peer4.arrFiles) {
					...
					connectionSocket.close();
						...
						//String downloadFolder = "F:/Arpita/IP/Project 1/RFC/Peer4";
						String currentDirectory = System.getProperty("user.dir");
						String myFolder = currentDirectory+File.separator+"Peer4"; 
...
public class Peer4 {
	...
	public static void main (String args[]) throws Exception {
			...
			String myPortNumber = "65405"; //Port number for Peer2
		...
		//Creating Local RFC-Index
				//File folder = new File("F:/Arpita/IP/Project 1/RFC/Peer4"); //Folder for Local RFC
			String currentDirectory = System.getProperty("user.dir");
			String myFolderPath = currentDirectory+File.separator+"Peer4";
				...
				File[] listOfFiles = folder.listFiles();
				   ...
				   for (int i = 0; i < listOfFiles.length; i++) {
				      if (listOfFiles[i].isFile()) {
				        ...
				        Peer4RFC peerObj = new Peer4RFC(rfc_number[0], listOfFiles[i].getName() ,myPortNumber); //Have to change
				        Peer4RFC.rfcList.add(peerObj);
				    ...
				    System.out.println("Local RFC Index" +Peer4RFC.rfcList.toString());
				    
				    //Thread for the server process
				    ServerPeer4 obj = new ServerPeer4();
				  ...
				  //To check if the application is terminated
					...
					class ShutdownHook  extends Thread 
					{
					    public void run() 
					    {
						  ...
						  Peer4.appActive = false;  // boolean to check if the application is active
		...
		if(activePeersPresent) {
					...
					if(portNumbersOfActivePeers != null) {
							for(int element : portNumbersOfActivePeers) {
								...
								for(String fileName : titleList) {
									String rfc_number[] = fileName.split("\\.");
									Peer4RFC peerObj = new Peer4RFC(rfc_number[0], fileName ,element+""); //Have to change
									Peer4RFC.rfcList.add(peerObj);
			...
			System.out.println(Peer4RFC.rfcList);
	...
	//Download file code
  	...
  	sc = new Scanner(System.in);
	reply = sc.next();
	Peer4RFC obj2 = new Peer4RFC();
	
	if(reply.equalsIgnoreCase("y")) {
	
	//Main download code
		
		boolean otherFilesTobeDownloaded = obj2.searchForFilesToDownload(Peer4RFC.rfcList,myPortNumber);
	

		if(otherFilesTobeDownloaded) {
	      	
			//String downloadFolder = "F:/Arpita/IP/Project 1/RFC/Peer4"; //Folder where the files will be downloaded
			...
			String downloadFolder = myFolderPath;
			  	...
			  	for(Peer4RFC abc : Peer4RFC.rfcList) 
			  	    {
			  	  ...
			  	  long time2 = System.currentTimeMillis();
			  	    ...
			  	    timeAppend += time3;
			  	  String fileToStoreTimeStamps = currentDirectory+File.separator+"Peer4.text";
			  	  PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter(fileToStoreTimeStamps, true)));
			  	  //PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("F:/Arpita/IP/Project 1/Time Calculated/Peer4_task1.txt", true)));
