...
class Peer3RFC  {
	...
	public Peer3RFC(String a, String b, String c) {
	...
	static LinkedList<Peer3RFC> rfcList = new LinkedList<Peer3RFC>();
	@Override
	...
	public  boolean searchForFilesToDownload(LinkedList<Peer3RFC> myRFCIndex, String myPortNumber) {
		boolean areThereAnyFiles =false;
		
		if(myRFCIndex!=null) {
		for(Peer3RFC element : myRFCIndex) {
...
class ServerPeer3 implements Runnable {
	
	public void run() {
		try {
			ServerSocket welcomeSocket = new ServerSocket(65404);
			...
			while(true) {
				...
				if(data.equalsIgnoreCase("GET RFC-INDEX")) {
					...
					outToClient.writeBytes("Size : "+Peer3.arrFiles.size() +'\n');
				
					for(String abc : Peer3.arrFiles) {
					...
					connectionSocket.close();
						...
						//String downloadFolder = "F:/Arpita/IP/Project 1/RFC/Peer3";
						String currentDirectory = System.getProperty("user.dir");
						String myFolder = currentDirectory+File.separator+"Peer3"; 
...
public class Peer3 {
	...
	public static void main (String args[]) throws Exception {
			...
			String myPortNumber = "65404"; //Port number for Peer2
			String rsHostname = "LAPTOP-V4IOJVM6";
		...
		//Creating Local RFC-Index
				//File folder = new File("F:/Arpita/IP/Project 1/RFC/Peer3"); //Folder for Local RFC
			String currentDirectory = System.getProperty("user.dir");
			String myFolderPath = currentDirectory+File.separator+"Peer3";
			...
			File[] listOfFiles = folder.listFiles();
				   ...
				   for (int i = 0; i < listOfFiles.length; i++) {
				      if (listOfFiles[i].isFile()) {
				        ...
				        Peer3RFC peerObj = new Peer3RFC(rfc_number[0], listOfFiles[i].getName() ,myPortNumber); //Have to change
				        Peer3RFC.rfcList.add(peerObj);
				    ...
				    System.out.println("Local RFC Index" +Peer3RFC.rfcList.toString());
				    
				    //Thread for the server process
				    ServerPeer3 obj = new ServerPeer3();
				  ...
				  //To check if the application is terminated
					...
					class ShutdownHook  extends Thread 
					{
					    public void run() 
					    {
						  ...
						  Peer3.appActive = false;  // boolean to check if the application is active
		...
		if(activePeersPresent) {
					...
					if(portNumbersOfActivePeers != null) {
							for(int element : portNumbersOfActivePeers) {
								...
								for(String fileName : titleList) {
									String rfc_number[] = fileName.split("\\.");
									Peer3RFC peerObj = new Peer3RFC(rfc_number[0], fileName ,element+""); //Have to change
									Peer3RFC.rfcList.add(peerObj);
			...
			System.out.println(Peer3RFC.rfcList);
	...
	//Download file code
  	...
  	sc = new Scanner(System.in);
	reply = sc.next();
	Peer3RFC obj2 = new Peer3RFC();
	
	if(reply.equalsIgnoreCase("y")) {
	
	//Main download code
		
		boolean otherFilesTobeDownloaded = obj2.searchForFilesToDownload(Peer3RFC.rfcList,myPortNumber);
	

		if(otherFilesTobeDownloaded) {
	      	
			//String downloadFolder = "F:/Arpita/IP/Project 1/RFC/Peer3"; //Folder where the files will be downloaded
			String downloadFolder = myFolderPath;
			  	...
			  	for(Peer3RFC abc : Peer3RFC.rfcList) 
			  	    {
			  	  ...
			  	  long time2 = System.currentTimeMillis();
			  	    ...
			  	    timeAppend += time3;
			  	  String fileToStoreTimeStamps = currentDirectory+File.separator+"Peer3.text";
			  	  PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter(fileToStoreTimeStamps, true)));
			  	  //PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("F:/Arpita/IP/Project 1/Time Calculated/Peer3_task1.txt", true)));
